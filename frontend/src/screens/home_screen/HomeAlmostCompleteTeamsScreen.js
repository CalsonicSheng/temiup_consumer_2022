...
import { getAlmostCompleteTeamsRequestHandler } from '../../data/slices/teamSlice';

export default function HomeAlmostCompleteTeamsScreen() {
  ...
  useEffect(() => {
    ...
    // need to decrement the selectedPageState by 1 in frontend before sending to backend | page count starts from 0 in backend controller logic while frontend starts at 1
    const selectedPageStateDecrement = selectedPageState - 1;
    dispatch(getAlmostCompleteTeamsRequestHandler([lowerCasedCategory, selectedPageStateDecrement]));
